    ...
    def clean(self):
        ...
        if user is None:
            if User.objects.filter(email=username_or_email).exists():
                ...
                if user is None:
                    raise forms.ValidationError('用户名或邮箱或者密码不正确')
                else:
...
class RegisterForm(forms.Form):
    ...
    def clean_password_again(self):
        ...
        pwd_a = self.cleaned_data['password_again']
        if pwd != pwd_a:
...
class ChangePasswordForm(forms.Form):
    ...
    new_password_again = forms.CharField(label='再次的新密码', widget=forms.PasswordInput(
